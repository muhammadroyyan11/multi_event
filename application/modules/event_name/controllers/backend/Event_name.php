...
class Event_name extends Backend
{
  ...
  {
    // $event_id = dec_url($event_id); // Decrypt the ID if needed

    // Get event details if necessary
    $data['event'] = $this->model->get_event($event_id);
  ...
  //       'field_options' => $field_options[$key]
  //     );

  //     // Save the form field data to the database
  //     $this->db->insert('event_form_fields', $data);
  //   }

  //   // Redirect back to the event list or a success page
  //   redirect('cpanel/event_name');
  // }
      ...
      foreach ($list as $row) {
        ...
        $rows[] = '
                  <div class="btn-group" role="group" aria-label="Basic example">
                      <a href="' . url("event_name/addForm/" . $row->id) . '" id="addForm" class="btn btn-secondary" title="' . cclang("add form") . '">
  ...
  function add()
  {
    ...
    $data = array(
      'action' => url("event_name/add_action"),
      'name' => set_value("name"),
  ...
  function add_action()
  {
    if ($this->input->is_ajax_request()) {
      ...
      $this->form_validation->set_rules("name", "* Event Name", "trim|xss_clean|required");
      ...
      $this->form_validation->set_rules("admin_id", "* Admin id", "trim|xss_clean|required");
  ...
  function update($id)
  {
    ...
    if ($row = $this->model->find(dec_url($id))) {
      ...
      $data = array(
        'action' => url("event_name/update_action/$id"),
